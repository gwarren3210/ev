...
describe("Cache Module", () => {
    ...
    describe("getCachedOfferData", () => {
        it("returns null when Redis is unavailable", async () => {
            ...
            const result = await getCachedOfferData("offer123");
        ...
        it("returns data on hit", async () => {
            ...
            expect(mockRedis.get).toHaveBeenCalledWith(`oddsshopper:offers:offer123`);
        });
    ...
    describe("setCachedOfferData", () => {
        it("does nothing when Redis is unavailable", async () => {
            mockGetRedisClient.mockImplementation(async () => null);
            await expect(setCachedOfferData("offer123", [mockOffer])).resolves.toBeUndefined();
        });

        it("stores data and hash", async () => {
            await setCachedOfferData("offer123", [mockOffer]);
        ...
        it("invalidates EV cache when hash changes", async () => {
            ...
            mockRedis.scan.mockImplementation(async () => ["0", ["ev:calc:offer123:req1"]]);

            await setCachedOfferData("offer123", [mockOffer]);

            expect(mockRedis.scan).toHaveBeenCalled();
            expect(mockRedis.del).toHaveBeenCalledWith("ev:calc:offer123:req1");
        });
    ...
    describe("invalidateEVCacheForOffer", () => {
        it("returns 0 when Redis is unavailable", async () => {
            mockGetRedisClient.mockImplementation(async () => null);
            const result = await invalidateEVCacheForOffer("offer123");
        ...
        it("deletes multiple keys using SCAN", async () => {
            ...
            const deletedCount = await invalidateEVCacheForOffer("offer123");
